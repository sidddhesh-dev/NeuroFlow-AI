...
class DocumentSerializer(serializers.ModelSerializer):
    ...
    def validate_file(self,value):
        ...
        if value.size>max_size:
            raise serializers.ValidationError("file size exceeds 100 mb")
        return value
    ...
    def get_filesize(self,obj):
        try:
            size=SupabaseStorageService.get_file_size(obj.file.name)
        except Exception:
            return "Unknown"
        if size<1024:
            return f"{size} B"
...
class ChatSessionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model=ChatSession
        fields=["id","title","document","document_name","created_at","updated_at","messages","is_pinned"]
