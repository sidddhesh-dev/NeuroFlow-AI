...
function History() {
    ...
    function handleContinue(item) {
        ...
        if (item.type === "document") {
            navigate("/documents", {
                state: { documentId: item.target_id },
            });
            return;
        }
    ...
    async function handleDelete(item) {
        const confirmed = await confirmDelete(`Delete this ${item.type}?`);
        if (!confirmed) return;
    ...
    return (
        <section className="history-page">
            <div className="history-header">
                <div>
                    <h1>History</h1>
                    <p>Continue your previous work.</p>
                </div>
            </div>
            ...
            <div className="history-list">
                ...
                {error && (
                    <p className="history-message">Failed to load history.</p>
                )}

                {!isLoading && !error && history.length === 0 && (
                    <p className="history-message">No history found.</p>
                )}
